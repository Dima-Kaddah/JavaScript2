...
// open and close reading elements
// sum all books prices
...
onlyReadBooks.addEventListener('click', () => {
  let ele = document.querySelectorAll('li');
  books.forEach((book, i) => {
    let eleIndex = ele[i];
    if (book.alreadyRead === false) {
      eleIndex.classList.toggle('close');
    }
  });
});

let alreadyReadPrice = [];
let notRead = [];
let allBooks = [];
let c = false;
let alreadyReadPriceAll = '';
let notReadYet = '';
let allBooksPrices = '';
let pricesH1 = document.createElement('h2');
pricesH1.innerText = allBooksPrices;
document.body.appendChild(pricesH1);
onlyReadBooks.addEventListener('click', showPrice);

function showPrice() {
  if (!c) {
    pricesH1.innerText = alreadyReadPriceAll;
    c = true;
  } else {
    pricesH1.innerText = allBooksPrices;
    c = false;
...
books.forEach(function(getAllBooks) {
  if (getAllBooks.alreadyRead === true) {
    alreadyReadPrice.push(getAllBooks.price);
    let TrueReading = alreadyReadPrice.reduce((total, element) => total + parseFloat(element));
    alreadyReadPriceAll = `Total price is : ${TrueReading}`;
  } else if (getAllBooks.alreadyRead === false) {
    notRead.push(getAllBooks.price);
    let notRe = notRead.reduce((total, element) => total + parseFloat(element));
    notReadYet = `Total price is : ${notRe}`;
  }
  allBooks.push(getAllBooks.price);
  let allprices = allBooks.reduce((total, element) => total + parseFloat(element));
  allBooksPrices = `Total price is : ${allprices}`;
});
